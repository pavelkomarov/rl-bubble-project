...
It does conditional image generation, using a diffusion model
...
lrate = 1e-4
save_model = False
save_dir = './data/diffusion_outputs40/'
...
class Bubbles(Dataset):
	...
	def __init__(self, fname):
		with open(fname, 'rb') as f:
			self.frame10s = np.load(f)
			self.xyr = np.load(f)

	def __getitem__(self, key):
		"""this returns data and label for a particular example"""
		data168x504 = np.zeros((1, 168, 504), dtype=np.float32) # the data has to have dimensions divisible by 4 to work with the NNs
		data168x504[0,:-1,:-7] = self.frame10s[key] # it also has to be type float32 
		return data168x504, self.xyr_to_class(self.xyr[key])
...
tf = transforms.Compose([transforms.ToTensor()]) # mnist is already normalised 0 to 1. So are my shockwave plots
...
for ep in range(n_epoch):
	...
	loss_ema = None
	real_examples = [[] for i in range(n_classes)]
	for x, c in pbar:
		optim.zero_grad()
		x = x.to(device)
		c = c.to(device)
		...
		# snag 4 real examples from each class for plotting purposes
		for k in range(n_classes):
			if len(real_examples[k]) < 1:
				idx = (c == k).nonzero()
				if idx.shape[0] > 1: idx = idx[0] # if more than one example, grab first
				if idx.shape[0] > 0: # if any examples
					real_examples[k].append(x[idx])
	...
	with torch.no_grad():
		...
		for w_i, w in enumerate(ws_test):
			x_gen, x_gen_store = ddpm.sample(n_sample, (1, 168, 504), device, guide_w=w)

			# append some real images at bottom, order by class also
			x_real = torch.Tensor(x_gen.shape).to(device)
			for k in range(n_classes):
				for j in range(1):
					if len(real_examples[k]) > 0:
						x_real[k+(j*n_classes)] = real_examples[k][j]
					else:
						x_real[k+(j*n_classes)] = 0
				...
				def animate_diff(i, x_gen_store):
					...
					for row in range(int(n_sample/n_classes)):
						for col in range(n_classes):
							axs[col].clear() # this was [row, col], but with only a single row pyplot fucks up
							axs[col].set_xticks([])
							axs[col].set_yticks([])
